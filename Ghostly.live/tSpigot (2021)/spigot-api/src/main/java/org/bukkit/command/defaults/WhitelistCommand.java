    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (args.length == 1) {
            if (args[0].equalsIgnoreCase("reload")) {
                ...
                Command.broadcastCommandMessage(sender, "&aReloaded white-list from file");
                return true;
            } else if (args[0].equalsIgnoreCase("on")) {
                if (badPerm(sender, "enable")) return true;

                Bukkit.setWhitelist(true);
                Command.broadcastCommandMessage(sender, "&aTurned on white-listing");
                return true;
            } else if (args[0].equalsIgnoreCase("off")) {
                if (badPerm(sender, "disable")) return true;

                Bukkit.setWhitelist(false);
                Command.broadcastCommandMessage(sender, "&aTurned off white-listing");
                return true;
                ...
                sender.sendMessage("&aWhite-listed players: &r" + result.toString());
                return true;
            }
            ...
            if (args[0].equalsIgnoreCase("add")) {
                ...
                Command.broadcastCommandMessage(sender, "&aAdded &r" + args[1] + "&a to white-list");
                return true;
                ...
                Command.broadcastCommandMessage(sender, "&aRemoved &r" + args[1] + "&a from white-list");
                return true;
            }
        }

        sender.sendMessage("&cCorrect command usage:\n" + usageMessage);
        return false;
    }

    private boolean badPerm(CommandSender sender, String perm) {
        if (!sender.hasPermission("bukkit.command.whitelist." + perm)) {
            sender.sendMessage("&cYou do not have permission to perform this action.");
            return true;
        }
