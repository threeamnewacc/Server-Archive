...
public class EffectCommand extends VanillaCommand {
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if ("clear".equalsIgnoreCase(args[1])) {
            ...
            sender.sendMessage(String.format("&cTook all effects from %s", args[0]));
            return true;
        }
        ...
        if (effect == null) {
            sender.sendMessage(ChatColor.RED + String.format("&cEffect, %s, not found", args[1]));
            return true;
        }
        ...
        if (duration_temp == 0) {
            if (!player.hasPotionEffect(effect)) {
                sender.sendMessage(String.format("&cCouldn't take %s from %s as they do not have the effect", effect.getName(), args[0]));
                return true;
            }

            player.removePotionEffect(effect);
            broadcastCommandMessage(sender, String.format("&cTook %s from %s", effect.getName(), args[0]));
        } else {
            final PotionEffect applyEffect = new PotionEffect(effect, duration, amplification);

            player.addPotionEffect(applyEffect, true);
            broadcastCommandMessage(sender, String.format("&aGiven &r%s &a(ID &r%d&a) * &r%d &ato &r%s &afor &r%d &aseconds", effect.getName(), effect.getId(), amplification, args[0], duration_temp));
        }
