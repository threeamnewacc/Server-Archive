...
import org.bukkit.command.ArgumentExecutor;
import org.bukkit.command.BaseCommand;
...
public abstract class JavaPlugin extends PluginBase {
    ...
    // Credits to allen dust on zSpigot.
    public JavaPlugin(String fakeName) {
    	this.isEnabled = true;
    	JavaPluginLoader javaPluginLoader = JavaPluginLoader.loader;
    	this.init(javaPluginLoader, javaPluginLoader.server, new PluginDescriptionFile(fakeName, "1.0", null), null, null, this.classLoader);
    }
    ...
    private PluginCommand getCommandNode(String name) {
    ...
    private void registerCommand(BaseCommand executor) {
		PluginCommand command = getCommandNode(executor.getName());
    	if(executor.getAliases() != null) {
    		command.setAliases(Arrays.asList(executor.getAliases()));
    	}
    ...
    public void registerCommands(Object... commands) {
    	for(Object command : commands) {
    		...
    		} else if(command instanceof ArgumentExecutor) {
    			registerCommand((ArgumentExecutor) command);
    		}
    	}
    }
    
    private void registerCommand(ArgumentExecutor executor) {
		PluginCommand command = getCommandNode(executor.getName());
    	if(executor.getAliases() != null) {
    		command.setAliases(Arrays.asList(executor.getAliases()));
    	}
