    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (player != null) {
            ...
            if (mode != player.getGameMode()) {
                ...
                if (mode != player.getGameMode()) {
                    sender.sendMessage("&cGame mode change for " + player.getName() + " failed!");
                } else {
                    if (player == sender) {
                        Command.broadcastCommandMessage(sender, "&aSet own game mode to &r" + mode.toString() + "&a mode");
                    } else {
                        Command.broadcastCommandMessage(sender, "&aSet &r" + player.getName() + "&a's game mode to &r" + mode.toString() + "&a mode");
                    }
                }
            } else {
                sender.sendMessage("&c" + player.getName() + " already has game mode " + mode.getValue());
            }
        } else {
            sender.sendMessage("&cFailed to find player '" + playerArg + "' on server.");
        }
