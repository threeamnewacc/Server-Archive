...
public class AchievementCommand extends VanillaCommand {
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (player == null) {
            sender.sendMessage("&cYou must specify which player you wish to perform this action on.");
            return true;
        }
        ...
        if (statisticString.equals("*")) {
            ...
            Command.broadcastCommandMessage(sender, String.format("&aSuccessfully given all achievements to &r%s", player.getName()));
            return true;
        }
        ...
        if (achievement != null) {
            if (player.hasAchievement(achievement)) {
                sender.sendMessage(String.format("&c%s already has achievement %s", player.getName(), statisticString));
                return true;
            }
            ...
            Bukkit.getServer().getPluginManager().callEvent(event);
            if (event.isCancelled()) {
                sender.sendMessage(String.format("&cUnable to award %s the achievement %s", player.getName(), statisticString));
                return true;
            }
            player.awardAchievement(achievement);
                
            Command.broadcastCommandMessage(sender, String.format("&aSuccessfully given &r%s &athe stat &r%s", player.getName(), statisticString));
            return true;
        }

        if (statistic == null) {
            sender.sendMessage(String.format("&cUnknown achievement or statistic '%s'", statisticString));
            return true;
        }
        ...
        if (statistic.getType() == Type.UNTYPED) {
            ...
            Bukkit.getServer().getPluginManager().callEvent(event);
            if (event.isCancelled()) {
                sender.sendMessage(String.format("&cUnable to increment %s for %s", statisticString, player.getName()));
                return true;
            }
