    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (args.length == 4) {
            if (world != null) {
                ...
                Command.broadcastCommandMessage(sender, "&aSet &r" + player.getDisplayName() + "&a's spawnpoint to &r" + x + ", " + y + ", " + z);
            }
        } else if (args.length <= 1) {
            Location location = player.getLocation();
            player.setBedSpawnLocation(location, true);
            Command.broadcastCommandMessage(sender, "&aSet &r" + player.getDisplayName() + "&a's spawnpoint to &r" + location.getX() + ", " + location.getY() + ", " + location.getZ());
        } else {
