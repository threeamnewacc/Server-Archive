...
public class TimeCommand extends VanillaCommand {
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        if (args.length < 2) {
            sender.sendMessage("&cIncorrect usage. Correct usage:\n" + usageMessage);
            return false;
        }
        ...
        if (args[0].equals("set")) {
            if (!sender.hasPermission("bukkit.command.time.set")) {
                sender.sendMessage("&cYou don't have permission to set the time");
                return true;
            }
            ...
            Command.broadcastCommandMessage(sender, "&aSet time to &r" + value);
            ...
            Command.broadcastCommandMessage(sender, "&aAdded &r" + value + "&a to time");
        } else {
            sender.sendMessage("&cUnknown method. Usage: " + usageMessage);
        }
