        ...
        if (args.length > 0) {
            ...
            if (world.isGameRule(rule)) {
                if (args.length > 1) {
                    ...
                    Command.broadcastCommandMessage(sender, "&aGame rule &r" + rule + "&a has been set to: &r" + value);
                } else {
                    String value = world.getGameRuleValue(rule);
                    sender.sendMessage(rule + " = " + value);
                }
            } else {
                sender.sendMessage("&cNo game rule called " + rule + " is available");
            }

            return true;
        } else {
            sender.sendMessage("&cUsage: " + usageMessage);
            sender.sendMessage("&cRules: " + this.createString(getGameWorld(sender).getGameRules(), 0, ", "));
