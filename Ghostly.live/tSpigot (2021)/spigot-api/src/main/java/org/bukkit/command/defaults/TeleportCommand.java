...
public class TeleportCommand extends VanillaCommand {
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        ...
        if (args.length < 3) {
            ...
            if (target == null) {
                sender.sendMessage("&cCan't find player " + args[args.length - 1] + ". No tp.");
                return true;
            }
            
            player.teleport(target, TeleportCause.COMMAND);
            Command.broadcastCommandMessage(sender, "&aTeleported &r" + player.getDisplayName() + "&a to &r" + target.getDisplayName());
            ...
            if (x == MIN_COORD_MINUS_ONE || y == MIN_COORD_MINUS_ONE || z == MIN_COORD_MINUS_ONE) {
                sender.sendMessage("&cPlease provide a valid location!");
                return true;
            }
            ...
            Command.broadcastCommandMessage(sender, String.format("&aTeleported &r%s &ato &r%.2f, %.2f, %.2f", player.getDisplayName(), x, y, z));
        }
        return true;
    }
