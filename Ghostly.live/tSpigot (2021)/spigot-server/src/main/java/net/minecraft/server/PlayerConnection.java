...
import me.enzol.spigot.event.player.PlayerAttackEvent;
import me.enzol.spigot.event.player.PlayerReachEvent;
		...
		if (this.b(packetplayinflying)) {
			...
			if (!this.player.viewingCredits) {
				...
				if (packetplayinflying.hasPos && delta > 0.0D && this.checkMovement && !this.player.dead) {
					for (MovementHandler handler : TrainingSpigot.INSTANCE.getMovementHandlers()) {
						try {
				...
				if (this.checkMovement && !this.player.dead) {
					...
					if (packetplayinflying.h() && entityPlayer.getDataWatcher().getFloat(6) > 0.0F) {
...
//    	if (this.networkManager.getProtocolVersion() != null && !this.networkManager.getProtocolVersion().isSupported(packet)) {
...
//    	
        ...
        try {
            ...
            for (PacketHandler handler : TrainingSpigot.INSTANCE.getPacketHandlers()) {
				try {
   ...
   private void handleCommand(String s) {
       ...
       // CraftBukkit start - whole method
        if ( org.spigotmc.SpigotConfig.logCommands ) this.minecraftServer.console.sendMessage(this.player.getName() + " issued server command: " + s);
        ...
        if (entity != null) {
            ...
            if (this.player.h(entity) < d0) {
                ...
                if (packetplayinuseentity.a() == PacketPlayInUseEntity.EnumEntityUseAction.INTERACT) {
                    ...
                    final PlayerAttackEvent event2 = new PlayerAttackEvent(entity.getBukkitEntity(), this.player.getBukkitEntity());
                	...
                	Bukkit.getPluginManager().callEvent(new PlayerReachEvent(this.getPlayer(), this.player.g(entity)));
                    this.getPlayer().setReachDistance(this.player.g(entity));
                }
            }
        }
