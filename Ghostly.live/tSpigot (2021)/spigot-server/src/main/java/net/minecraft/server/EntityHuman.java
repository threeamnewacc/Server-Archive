            ...
            if ((f > 0.0F) || (f1 > 0.0F)){
                ...
                if (flag2)
                {
                    if (entity instanceof EntityPlayer && entity.velocityChanged){
                        ...
                        KnockbackProfile kb = getKnockback();

                        float kbV2;

                        double velY = 0.0D;
                        double maxVelY = kb.kbV.value;
                        double velX;
                        double velZ;
                        if (kb.inheritVelocity.value){
                            double entityVelX = entity.motX * kb.friction.value;
                            double entityVelZ = entity.motZ * kb.friction.value;
                            if (kb.yawBasedCalculation.value){
                                ...
                                velX = entityVelX + v.getX();
                                velZ = entityVelZ + v.getZ();
                            }
                            if (kb.floatyV.value) {
                                velY += (entity.motY > velY ? maxVelY : maxVelY - entity.motY);
                            }
                        }
                        else{
                            if (kb.yawBasedCalculation.value){
                        ...
                        if (kb.inheritVelocity.value){
                            if ((kb.vLimit.value > 0.0F) && (yOff >= kb.vLimit.value))
                            {
                                double d = kb.vLimit.value / yOff;
                                velY *= d;
                            }
                        }
                        else{
                            if (yOff > 2.5D) {
                                kb.ticksDown.value = MinecraftServer.currentTick;
                            }
                            if ((yOff > 2.5D) || (MinecraftServer.currentTick - kb.ticksDown.value < 10.0F)) {
                                velY = -0.05D;
                            }
                        }
                        velX *= kb.kbX.value;
                        velZ *= kb.kbZ.value;
                        if (this.wtapValue != 0){
                            velX *= kb.kbWtapH.value;
                            velZ *= kb.kbWtapH.value;
                            if (kb.kbWtapVMult.value) {
                                velY *= kb.kbWtapV.value;
                            } else {
                                velY += kb.kbWtapV.value;
                            }
                        }
                        if (isSprinting()){
                            this.motX *= kb.sprintSlowdown.value;
                            this.motZ *= kb.sprintSlowdown.value;

                            this.wtapValue = 0;
                        }

                        double highestBlockAt = victim.getBukkitEntity().getLocation().getY() - ((EntityPlayer)victim).getBukkitEntity().getWorld().getHighestBlockAt(((EntityPlayer)victim).getBukkitEntity().getLocation()).getLocation().getY();

                        if(isInCombo && highestBlockAt >= kb.alturaCombo.value){
                            velY = kb.verticalCombo.value;
                        }
                        victim.fallDistance = 0.0F;
                        victim.playerConnection.sendPacket(new PacketPlayOutEntityVelocity(entity.getId(), velX, velY, velZ));
