...
public abstract class World implements IBlockAccess {
    ...
    public void entityJoinedWorld(Entity entity, boolean flag) {
        ...
        // Spigot start
        if (TrainingSpigot.INSTANCE.getConfig().isEntityActivation() && !org.spigotmc.ActivationRange.checkIfActive(entity)) {
            entity.ticksLived++;
            entity.inactiveTick();
        ...
        for (int k2 = i; k2 < j; ++k2) {
            for (int l2 = k; l2 < l; ++l2) {
                for (int i3 = i2; i3 < j2; ++i3) {
                    Block block = this.getType(blockposition_mutableblockposition.c(k2, l2, i3)).getBlock();
                    if (block.getMaterial() != Material.AIR) {
                        return true;
                    }
                }
