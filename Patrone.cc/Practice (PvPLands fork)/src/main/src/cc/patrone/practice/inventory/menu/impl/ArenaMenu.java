...
public class ArenaMenu extends Menu {
	...
	public ArenaMenu(PracticePlugin plugin) {
		super(3, "Select an Arena to Duel On");
		this.plugin = plugin;
	}
	...
	private Action getAction(Arena arena) {
		return player -> {
			...
			Arena duelArena = arena == null ? plugin.getArenaManager().getRandomArena(profile.getSelectedKit()) : arena;
