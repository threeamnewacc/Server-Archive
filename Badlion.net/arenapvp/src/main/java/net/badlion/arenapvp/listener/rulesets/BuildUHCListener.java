...
public class BuildUHCListener implements Listener {
	...
	public void onKitLoadEvent(KitLoadEvent event) {
		if (KitRuleSet.buildUHCRuleSet == event.getKitRuleSet()) {
	...
	public void onArrowHit(EntityDamageByEntityEvent event) {
		if (event.getEntity() instanceof Player && event.getDamager() instanceof Arrow) {
			...
			if (MatchState.playerIsInMatchAndUsingRuleSet(damaged, KitRuleSet.buildUHCRuleSet)) {
	...
	public void onHealthRegenEvent(EntityRegainHealthEvent event) {
		if (event.getEntity() instanceof Player) {
			...
			if (MatchState.playerIsInMatchAndUsingRuleSet(player, KitRuleSet.buildUHCRuleSet)) {
	...
	public void onPlayerBucketFillEvent(PlayerBucketFillEvent event) {
		...
		} else if (MatchState.playerIsInMatchAndUsingRuleSet(player, KitRuleSet.buildUHCRuleSet)) {
				...
				if (block.getY() > max) {
					...
					if (event.getBucket() == Material.WATER_BUCKET || event.getBucket() == Material.LAVA_BUCKET) {
						if (player.getLocation().getY() > max + 2) {
							...
							player.sendFormattedMessage("{0}Water and lava glitching is not allowed!", ChatColor.RED);
						}
					}
				}
			}
