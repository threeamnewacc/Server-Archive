...
public class PartyPlayerInventoriesInventory {
	...
	public static void openViewPartyPlayerInventoriesInventory(Player player, Group group) {
		...
		// Get enemy party
	    /* TODO: Redo this once api stuff is done since inventory will be captured on the arenapvp server
	...
	private static void openPlayerInventory(SmellyInventory smellyInventory, Player player, ItemStack item) {
		...
		Player viewed = ArenaLobby.getInstance().getServer().getPlayerExact(name);

		if (viewed == null) {
			player.sendFormattedMessage("{0} has logged off!", ChatColor.RED + name);
		...
		// Are they alive?
		/* TODO: Redo once api is done
		Group group = ArenaLobby.getInstance().getPlayerGroup(viewed);
