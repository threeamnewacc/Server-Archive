...
public class KitPreviewInventory {
	...
	public static void openKitPreviewInventory(final Player sender, final KitRuleSet kitRuleSet, final SmellyInventory smellyInventory, Inventory currentInventory) {
		...
		final PotPvPPlayer potPvPPlayer = PotPvPPlayerManager.getPotPvPPlayer(sender.getUniqueId());
		if (!potPvPPlayer.isKitsLoaded()) {
			if (!potPvPPlayer.isLoadingKits()) {
				...
				new BukkitRunnable() {
					...
					public void run() {
						try {
							KitCommon.inventories.put(sender.getUniqueId(), KitCommon.getAllKitContents(Gberry.getConnection(), sender.getUniqueId()));
						...
						new BukkitRunnable() {
							...
							public void run() {
								if (sender != null && sender.isOnline()) {
									if (newSmellyInventory != null && newSmellyInventory.getMainInventory().getViewers().contains(sender)) {
										KitPreviewInventory.fillPreviewInventory(sender, kitRuleSet, newSmellyInventory.getMainInventory(), -1, hasSubMenu);
									}
