...
public class SkyWarsArena extends Arena {
    ...
    public SkyWarsArena(String arenaName, Location warp1, Location warp2, String extraData) {
        ...
        if (jsonFile.exists()) {
            ...
            PotPvP.getInstance().getLogger().info("File missing for SkyWarsArena " + arenaName);
        }
    }

    @Override
    public void scan() {
        Location warp = this.getWarp1();
    ...
    public void rebuild() {
        ...
        new RebuildArenaTask().runTaskTimer(PotPvP.getInstance(), 5, 1L);

        this.locationsToDestroy = new ArrayList<>(this.locationsToDestroyCopy);
    }

    public void startArenaUse(Game game) {
        this.destroyArenaTask = new DestroyArenaTask().runTaskTimer(PotPvP.getInstance(), 60 * 20, 1);

        this.match = (Match) game;

        this.warningTask = new BukkitRunnable() {
            public void run() {
                SkyWarsArena.this.match.sendMessage(ChatColor.DARK_AQUA + "Spawn Islands will be destroyed in 10 seconds.");
            }
        }.runTaskLater(PotPvP.getInstance(), 50 * 20);
    }
    ...
    private class DestroyArenaTask extends BukkitRunnable {
        ...
        public void run() {
            if (!this.initialized) {
                ...
                SkyWarsArena.this.match.sendMessage(ChatColor.DARK_AQUA + "The spawn islands have begun falling apart.");
                return;
            }

	        MassBlockUpdate massBlockUpdate = new CraftMassBlockUpdate(PotPvP.getInstance().getSpawnLocation().getWorld());
