package net.badlion.potpvp.states.kits;
...
public class KitCreationState extends GState<Group> implements Listener {
	...
	public void before(Group element) {
		...
		if (!this.getKitCreator(element.getLeader()).getKitRuleSet().usesCustomChests()) {
			element.getLeader().sendMessage(ChatColor.YELLOW + "Cannot edit kit contents or enchants in this kit, can only move items around.");
		}
	...
	public void after(Group element) {
		...
		ConcurrentLinkedQueue<Location> locations = PotPvP.getInstance().getBlockedBlockChangeLocations().get(player);
    ...
    public static void teleportToKitCreationArea(Player player) {
        ...
        player.sendMessage(ChatColor.AQUA + "Welcome to the Kit Creation area, open the chest to customize your kit!");
    }
                ...
                if (event.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
                    if (block.getType().equals(Material.WALL_SIGN) || block.getType().equals(Material.SIGN_POST)) {
                        ...
                        if (KitCreationHelper.isSaveKitSignLocation(block)) { // Save kit sign
                            if (kitCreator.getKitRuleSet() instanceof CustomRuleSet) {
                                if (kitCreator.getKitRuleSet().getClass() == CustomRuleSet.class) {  // Custom kits
                                    ...
                                    // Open the select event kit to save inventory
	                                CustomKitCreationInventories.openCustomKitSaveInventory(player, KitRuleSet.eventRuleSet);
                                }
                            } else {
                                KitHelper.saveKit(player, kitCreator.getKitRuleSet());
                            }
                            ...
                            if (kitCreator.getKitRuleSet() instanceof CustomRuleSet) {
                                if (kitCreator.getKitRuleSet().getClass() == CustomRuleSet.class) { // Custom kits
                                    ...
                                    // Open the select event kit to load inventory
	                                CustomKitCreationInventories.openCustomKitLoadInventory(player, KitRuleSet.eventRuleSet);
                                }
                            } else {
                                KitHelper.loadKit(player, kitCreator.getKitRuleSet());
                            }
                            ...
                            if (!(kitCreator.getKitRuleSet() instanceof CustomRuleSet)) {
                                ...
                                KitHelper.loadDefaultKit(player, kitCreator.getKitRuleSet(), true);
                            }
                        ...
                        if (kitCreator.getKitRuleSet().usesCustomChests()) {
                            ...
                            player.sendMessage(ChatColor.RED + "Cannot edit items in " + kitCreator.getKitRuleSet().getName() + " kit.");
                        }
                    }
                }
            }
