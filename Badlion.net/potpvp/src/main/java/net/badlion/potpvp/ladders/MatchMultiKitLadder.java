...
public class MatchMultiKitLadder extends Ladder {
    ...
    public MatchMultiKitLadder(int ladderId, List<KitRuleSet> kitRuleSets, MatchMakingService matchMakingService, LadderType ladderType,
    ...
    public Game createGame() {
        int kitNumber = MatchMultiKitLadder.random.nextInt(this.getAllKitRuleSets().size());

        return new Match(null, this.isRanked, this.getAllKitRuleSets().get(kitNumber)); // Temp set arena to null
    }

    @Override
    public boolean addPlayersToGame(Game game) {
        if (!(game instanceof Match)) {
            throw new RuntimeException("Invalid game passed to MatchMultiKitLadder " + this.ladderId);
        }
        ...
        Arena arena;
        try {
            arena = ArenaManager.getArena(match.getKitRuleSet().getArenaType());
