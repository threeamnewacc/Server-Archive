...
public class RelationUtil {
	...
	public static String describeThatToMe(RelationParticipator that, RelationParticipator me, boolean ucfirst) {
		...
		if (that instanceof Faction) {
			if (me instanceof FactionPlayer && myFaction == thatFaction) {
				ret = "your faction";
			} else {
				ret = thatFaction.getTag();
			}
		} else if (that instanceof FactionPlayer) {
			FactionPlayer fplayerthat = (FactionPlayer) that;
			if (that == me) {
				ret = "you";
			} else {
	...
	public static Faction getFaction(RelationParticipator rp) {
		...
		if (rp instanceof FactionPlayer) {
			return ((FactionPlayer) rp).getFaction();
		}
