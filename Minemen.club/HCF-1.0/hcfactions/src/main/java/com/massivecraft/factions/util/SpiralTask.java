...
public abstract class SpiralTask implements Runnable {
	...
	public SpiralTask(FLocation fLocation, int radius) {
		...
		if (this.world == null) {
			HCFactions.getInstance().log(Level.WARNING, "[SpiralTask] A valid world must be specified!");
			this.stop();
			return;
		}
		...
		// getAll this party started
		this.setTaskID(Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask(HCFactions.getInstance(), this, 2, 2));
	}
	...
	/*
	 * This is where the necessary work is done; you'll need to override this method with whatever you want done at each chunk in the spiral pattern. Return false if the entire task needs to be aborted, otherwise return true to continue.
	 */
	...
	/*
	 * Returns a Location pointing at the current chunk X and Z values. note that the Location is at the corner of the chunk, not the center.
	 */
	...
	public void finish() {
		// HCFactions.getInstance().log("SpiralTask successfully completed!");
		this.stop();
	}
