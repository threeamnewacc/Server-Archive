	...
	public void perform() {
		...
		if (!success.isEmpty()) {
			if (sender instanceof Player) {
				sendMessage(success);
				HCFactions.getInstance().log(success + " Command was run by " + fme.getName() + ".");
			} else // using HCFactions.getInstance().log() instead of sendMessage if run from server console so that "[Factions v#.#.#]" is prepended in server log
			{
				HCFactions.getInstance().log(success);
			}
		}
		// save change to disk
		Conf.save();
