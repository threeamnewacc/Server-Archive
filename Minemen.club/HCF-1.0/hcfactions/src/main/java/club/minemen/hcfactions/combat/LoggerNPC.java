...
import club.minemen.hcfactions.combat.events.CombatTagDestroyEvent;
import club.minemen.hcfactions.combat.events.CombatTagDropInventoryEvent;
...
public class LoggerNPC {
	...
	public LoggerNPC(UUID uuid, String name, Location location) {
		...
		entity.setMetadata("CombatLoggerNPC", new FixedMetadataValue(HCFactions.getInstance(), this.uuid));
		...
		entity.setMetadata("CombatLoggerInventory", new FixedMetadataValue(HCFactions.getInstance(), new ItemStack[36]));
		entity.setMetadata("CombatLoggerArmorInventory", new FixedMetadataValue(HCFactions.getInstance(), new ItemStack[4]));
		this.entity = entity;

		// Add to our map
		HCFactions.getInstance().getCombatTagManager().addCombatTagLogger(this.uuid, this);
	...
	public LoggerNPC(Player player, String namePrefix) {
		...
		EntityUtil.clearAI(entity);
		entity.setCustomName(namePrefix + player.getDisplayName());
		entity.setCustomNameVisible(true);
		...
		entity.getEquipment().setItemInHandDropChance(0F);
		entity.setCanPickupItems(false);
		entity.setMetadata("CombatLoggerInventory", new FixedMetadataValue(HCFactions.getInstance(), player.getInventory().getContents()));
		entity.setMetadata("CombatLoggerArmorInventory", new FixedMetadataValue(HCFactions.getInstance(), player.getInventory().getArmorContents()));
		this.entity = entity;
		this.player = player;
	...
	public void remove(REMOVE_REASON reason) {
		...
		HCFactions.getInstance().getCombatTagManager().removeCombatTagged(this.uuid);
	}
	...
	public void setArmor(ItemStack[] contents) {
		this.entity.setMetadata("CombatLoggerArmorInventory", new FixedMetadataValue(HCFactions.getInstance(), contents));
	}
	...
	public void setInventory(ItemStack[] contents) {
		this.entity.setMetadata("CombatLoggerInventory", new FixedMetadataValue(HCFactions.getInstance(), contents));
	}
