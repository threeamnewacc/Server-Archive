		...
		try {
			...
			try {
				...
				if (callback instanceof ErrorCallback) {
					this.runTask(() -> ((ErrorCallback) callback).onError("Error connecting to " + post.getURI().getHost() + " : " + e.getMessage()));
				}
			...
			if (line != null) {
				...
				if (code != 200) {
					...
					System.out.println("From api: " + url);
			...
			if (entity != null) {
				try (BufferedReader reader = new BufferedReader(new InputStreamReader(entity.getContent()))) {
					...
					if (callback instanceof ErrorCallback) {
						this.runTask(() -> ((ErrorCallback) callback).onError("Error parsing Json: " + e.getMessage()));
					}
				}
			}
		}
