...
public class UnblacklistCommand extends BaseCommand {
    ...
    @Command(name = "unblacklist", rank = Rank.ADMINISTRATOR)
    public void onCommand(CommandArgs command) {
        ...
        if (args.length < 2) {
            ...
            Clickable clickable = new Clickable(ChatColor.RED + "Usage: /unblacklist <target> [reason] [-s]", ChatColor.YELLOW + "Remove a player's blacklist.\nAdd a \"-s\" at the end to silently ban a player.", "");
        ...
        new BukkitRunnable() {
            public void run() {
                new PunishmentHelper(sender, args[0], PunishmentType.BLACKLIST, reason, (long) -1, silent, true);
            }
